...
def ShowResults(img, image_file, results, labelmap, threshold=0.6, save_fig=False):
    ...
    if save_fig:
        plt.savefig(image_file[:-4] + '_dets2.jpg', bbox_inches="tight")
        print('Saved: ' + image_file[:-4] + '_dets2.jpg')
    # plt.show()
    try:
        plt.show()
...
if __name__ == '__main__':
    ...
    im_names = os.listdir('examples/time_images')
    times = []
    for im_name in im_names:
        time_s = time.time()
        image_file = 'examples/time_images/' + im_name
    ...
    cnt = 0
    out_filepath = os.path.join("examples", "times", f"time_{cnt:05}.csv")
    while os.path.isfile(out_filepath):
        cnt += 1
        out_filepath = os.path.join("examples", "images", f"time_{cnt:05}.csv")
    with open(out_filepath, "w") as o:
